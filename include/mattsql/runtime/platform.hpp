...
class RuntimePageAllocationHandle;

enum class LogLevel { Trace, Debug, Info, Warn, Error, Fatal };

enum class IoOperation { Read, Write, Flush };
...
public:
  ...
  virtual Result<RuntimePageAllocation> AllocatePages(std::size_t page_count,
  ...
  /// Allocates runtime-managed pages and returns a move-only owner.
  Result<RuntimePageAllocationHandle> AllocatePageSpan(std::size_t page_count,
                                                       std::size_t alignment,
...
class RuntimePageAllocationHandle {
public:
  RuntimePageAllocationHandle() = default;
  RuntimePageAllocationHandle(PlatformRuntime &runtime,
                              RuntimePageAllocation allocation);
  ~RuntimePageAllocationHandle();

  RuntimePageAllocationHandle(const RuntimePageAllocationHandle &) = delete;
  RuntimePageAllocationHandle &operator=(const RuntimePageAllocationHandle &) = delete;

  RuntimePageAllocationHandle(RuntimePageAllocationHandle &&other) noexcept;
  RuntimePageAllocationHandle &operator=(RuntimePageAllocationHandle &&other) noexcept;

  [[nodiscard]] explicit operator bool() const {
    return runtime_ != nullptr && allocation_.data != nullptr;
  }

  [[nodiscard]] const RuntimePageAllocation &allocation() const { return allocation_; }

  [[nodiscard]] RuntimePageAllocation &allocation() { return allocation_; }

  [[nodiscard]] void *data() const { return allocation_.data; }
  [[nodiscard]] std::size_t page_count() const { return allocation_.page_count; }
  [[nodiscard]] std::size_t page_size() const { return allocation_.page_size; }
  [[nodiscard]] std::span<std::byte> bytes() const;

  [[nodiscard]] RuntimePageAllocation Release();
  Status Reset();

private:
  PlatformRuntime *runtime_ = nullptr;
  RuntimePageAllocation allocation_;
};
