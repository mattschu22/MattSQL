...
[[nodiscard]] bool require_boolean(const Value& value,
                                   std::string_view context) {
...
[[nodiscard]] int compare_values(const Value& left, const Value& right) {
  ...
  const auto left_is_integral = std::holds_alternative<std::int64_t>(left) ||
...
[[nodiscard]] Value evaluate_binary(const BinaryExpression& expression,
  ...
  if (expression.op == BinaryOperator::And) {
    ...
    return require_boolean(evaluate_expression(*expression.right, context),
                           "AND");
  }
  ...
  case BinaryOperator::Add:
    return require_integer(left, "addition") +
           require_integer(right, "addition");
  case BinaryOperator::Subtract:
    return require_integer(left, "subtraction") -
           require_integer(right, "subtraction");
...
[[nodiscard]] Value coerce_for_column(const Value& value,
...
QueryResult Engine::execute(std::string_view sql) {
  ...
  if (const auto* create =
    ...
    for (std::size_t index = 0; index < create->columns.size(); ++index) {
      if (index > std::numeric_limits<ColumnId>::max()) {
        throw std::runtime_error("too many columns in table: " +
                                 create->table_name);
      }
  ...
  if (const auto* insert =
          dynamic_cast<const InsertStatement*>(statement.get())) {
  ...
  for (std::size_t index = 0; index < select->projections.size(); ++index) {
    ...
    if (!projection.alias.empty()) {
      ...
      result.columns.push_back(
          default_projection_name(*projection.expression, index));
    }
  }
