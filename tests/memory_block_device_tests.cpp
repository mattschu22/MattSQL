...
TEST_CASE(memory_block_device_reports_geometry_and_reads_zeroes) {
  ...
  EXPECT_TRUE(mattsql::status_ok(device.Read(0, test::mutable_view(output))));
  test::expect_bytes_equal(output, filled_block(32, 0));
}
...
TEST_CASE(memory_block_device_writes_and_reads_aligned_blocks) {
  ...
  EXPECT_TRUE(mattsql::status_ok(device.Write(0, test::const_view(first))));
  EXPECT_TRUE(mattsql::status_ok(device.Write(32, test::const_view(second))));
  ...
  test::expect_bytes_equal(output, expected);
}
...
TEST_CASE(memory_block_device_rejects_invalid_accesses) {
  ...
  EXPECT_TRUE(mattsql::status_ok(device.Write(0, test::const_view(original))));

  EXPECT_TRUE(device.Read(1, test::mutable_view(output)).code ==
              mattsql::ErrorCode::InvalidArgument);
  EXPECT_TRUE(device.Read(0, test::mutable_view(short_buffer)).code ==
              mattsql::ErrorCode::InvalidArgument);
  EXPECT_TRUE(device.Read(96, test::mutable_view(long_buffer)).code ==
              mattsql::ErrorCode::InvalidArgument);
  EXPECT_TRUE(device.Write(1, test::const_view(replacement)).code ==
              mattsql::ErrorCode::InvalidArgument);
  EXPECT_TRUE(device.Write(0, test::const_view(short_buffer)).code ==
              mattsql::ErrorCode::InvalidArgument);
  ...
  EXPECT_TRUE(mattsql::status_ok(device.Read(0, test::mutable_view(output))));
  test::expect_bytes_equal(output, original);
}
...
TEST_CASE(memory_block_device_rejects_invalid_geometry) {
  ...
  EXPECT_TRUE(zero_size.Read(0, test::mutable_view(buffer)).code ==
              mattsql::ErrorCode::InvalidArgument);

  EXPECT_TRUE(!zero_block.IsValid());
  EXPECT_EQ(zero_block.BlockSize(), 0U);
  EXPECT_TRUE(zero_block.Write(0, test::const_view(buffer)).code ==
              mattsql::ErrorCode::InvalidArgument);
