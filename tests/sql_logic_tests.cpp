...
std::optional<mattsql::ErrorCode> parse_status_name(std::string_view name) {
  if (name == "InvalidArgument") {
    return mattsql::ErrorCode::InvalidArgument;
  }
  if (name == "NotFound") {
    return mattsql::ErrorCode::NotFound;
  }
  if (name == "AlreadyExists") {
    return mattsql::ErrorCode::AlreadyExists;
  }
  if (name == "TypeMismatch") {
    return mattsql::ErrorCode::TypeMismatch;
  }
  if (name == "ParseError") {
    return mattsql::ErrorCode::ParseError;
  }
  if (name == "BindError") {
    return mattsql::ErrorCode::BindError;
  }
  if (name == "PlanError") {
    return mattsql::ErrorCode::PlanError;
  }
  if (name == "ExecutionError") {
    return mattsql::ErrorCode::ExecutionError;
  }
  if (name == "IoError") {
    return mattsql::ErrorCode::IoError;
  }
  if (name == "Corruption") {
    return mattsql::ErrorCode::Corruption;
  }
  if (name == "TransactionConflict") {
    return mattsql::ErrorCode::TransactionConflict;
  }
  if (name == "NotSupported") {
    return mattsql::ErrorCode::NotSupported;
  }
  if (name == "Internal") {
    return mattsql::ErrorCode::Internal;
  }
  return std::nullopt;
}
...
LogicCase parse_header(const std::filesystem::path &path, std::size_t line,
  ...
  auto code = parse_status_name(code_name);
  if (!code.has_value()) {
...
std::string format_value(const mattsql::Value &value) {
...
std::vector<std::string> format_query_result(const mattsql::QueryResult &result) {
  ...
  for (const auto &row : result.rows) {
    ...
    for (const auto &value : row) {
      fields.push_back(format_value(value));
    }
...
void run_logic_directory(const std::filesystem::path &directory) {
...
TEST_CASE(sql_logic_integration_scripts_pass) {
  run_logic_directory(std::filesystem::path(MATTSQL_TEST_SOURCE_DIR) / "tests" /
                      "sqllogic");
}
...
TEST_CASE(sql_logic_deterministic_fuzzer_matches_model) {
  for (std::uint64_t seed_index = 0; seed_index < 8U; ++seed_index) {
    run_sql_logic_fuzzer_seed(std::uint64_t{0x5EED0000} + seed_index);
  }
}
